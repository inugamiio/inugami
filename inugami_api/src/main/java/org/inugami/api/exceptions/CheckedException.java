...
public class CheckedException extends Exception implements ExceptionWithErrorCode {
    ...
    public CheckedException(final int code, final String message, final Throwable cause) {
        this(ErrorCodeBuilder.newBuilder().setStatusCode(code).build(),cause, message,null);
    }
    ...
    public CheckedException(final ErrorCode errorCode, final Throwable cause, final String message, final Object... values) {
        super(MessagesFormatter.format(message, values), cause);
        this.errorCode = errorCode == null ? ErrorCodeBuilder.buildUndefineError() : errorCode;
    }
