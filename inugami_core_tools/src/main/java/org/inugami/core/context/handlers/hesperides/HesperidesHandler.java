...
import org.inugami.api.exceptions.services.HandlerException;
...
import org.inugami.core.context.handlers.hesperides.model.Property;
...
public class HesperidesHandler implements Handler {
    ...
    private Module findModule(final List<Module> modules, final String componentName) {
        Module result;
        // formatter:off
        result = modules.stream().filter(module -> getComponentName(module.getPath()).equals(componentName)).findFirst().orElse(null);
        // formatter:on
    ...
    private String findProperty(final Properties properties, final String propertyKey) {
        ...
        // formatter:off
        final Property propertyValue = propertiesList.stream().filter(property -> property.getName().equals(propertyKey)).findFirst().orElse(null);
        // formatter:on
