 ...
 * @since 13 mars 2017
 */
@SuppressWarnings({"java:S1845", "java:S2245", "java:S5361", "java:S2153", "java:S3655", "java:S1125", "java:S1130"})
public class MockJsonHelper {
    ...
    private static final int                  LAST_RANDOM_INDEX = -1;
    private              int                  index             = 0;
    ...
    public String getDataRandom(final String name) {
        ...
        if (!values.isEmpty()) {
            ...
            result = data.get(values.get(currentIndex));
        }
        return result;
    }

    private int grabFileIndex(final String name, final int size) {
        Integer currentIndex = cycleIndexies.get(name);
        ...
        cycleIndexies.put(name, currentIndex);
        return currentIndex;
    }
    ...
    private void manageLatency() {
        if (latencyEnable) {
            final long timeToSleep = Math.round(Double.valueOf(latency) * RANDOM.nextDouble());
    ...
    protected int grabRandomIndex(final int maxValue) {
        ...
        if (maxValue > 0) {
            ...
            final double currentRandom = Math.random() * (max + 1);
            double       round         = Math.round(currentRandom - 1);
            if (round < 0) {
                round = 0;
            }
            result = (int) round;
