 ...
 * @since 16 déc. 2016
 */
@SuppressWarnings({"java:S3655", "java:S3824", "java:S6355", "java:S1123", "java:S1874", "java:S1612", "java:S4034", "java:S1133"})
...
public class Context implements ApplicationContext,
    ...
    public void registerForShutodown(final BootstrapContext subContext) {
        Asserts.assertNotNull("sub context is mandatory", subContext);
    ...
    protected static synchronized ApplicationContext getInstance(final EngineListener listener,
    ...
    public FutureData<ProviderFutureResult> processPluginEvent(final String eventName, final Gav gav) throws Exception {
        Asserts.assertNotEmpty("event name mustn't be null!", eventName);
        ...
        Asserts.assertTrue(String.format("plugin not found !(%s)", gav.getHash()), foundPlugin.isPresent());
        final Plugin plugin     = foundPlugin.get();
        final String pluginHash = plugin.getGav().getHash();

        Asserts.assertTrue(String.format("plugin has no one event!(%s)", pluginHash), plugin.getEvents().isPresent());
        ...
        plugin.getEvents().get()
              ...
              .forEach(events::add);
        //@formatter:on
        Asserts.assertFalse(String.format("no event define in plugin %s !", pluginHash), events.isEmpty());

        final Optional<GenericEvent> event = events.stream()
                                                   .filter(item -> item.getName().equals(eventName))
                                                   .findFirst();

        Asserts.assertTrue(String.format("event (%s) not found in plugin %s !", eventName, pluginHash), event.isPresent());
    ...
    public void disablePlugin(final Gav gav) {
        getPlugin(gav).ifPresent(Plugin::disablePlugin);
    }

    @Override
    public void enablePlugin(final Gav gav) {
        getPlugin(gav).ifPresent(Plugin::enablePlugin);
    }
    ...
    public <T extends NamedComponent> T getNamedComponent(final String name,
        ...
        Asserts.assertNotNull("NamedComponent name mustn't be null!", name);
        ...
        Asserts.assertTrue(String.format("NamedComponent \"%s\" doesn't exists!", name), component.isPresent());
    ...
    public void setStartDayTime(final String dayTime) {
        Asserts.assertNotNull("Error day start time musn't be null!", dayTime);
