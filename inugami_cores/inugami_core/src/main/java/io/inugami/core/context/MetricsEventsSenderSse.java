...
public class MetricsEventsSenderSse extends Thread {
    ...
    private static final EventState[] TYPE = {};
    ...
    private static final int DELAIS_SIZE = 13;
        ...
        if (!result && (states != null)) {
            ...
            if (!result) {
                for (int i = previousStates.length - 1; i >= 0; i--) {
                    ...
                    final EventState currentPrevious = previousStates[i];

                    result = (current.isRunning() != currentPrevious.isRunning()) ||
                             (current.getEnd() != currentPrevious.getEnd());
                    if (result) {
                        break;
                    }
                }
    ...
    private void traceCsv(final Set<EventState> states) {
        ...
        for (final EventState event : states) {
            ...
            final long   delayValue = event.isRunning() ? 0 : event.getDelais();
            final String delay      = String.valueOf(delayValue);
            format(CSV, delay, DELAIS_SIZE);
