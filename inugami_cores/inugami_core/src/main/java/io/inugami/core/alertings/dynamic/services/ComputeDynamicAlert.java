...
@SuppressWarnings({"java:S2789", "java:S3655", "java:S1135"})
public class ComputeDynamicAlert {

    // =========================================================================
    // METHODS
    // =========================================================================
    public List<AlertingResult> compute(final SimpleEvent event,
    ...
    // OVERRIDES
    // =========================================================================
    protected List<AlertingResult> process(final SimpleEvent event,
                                           final JsonObject data,
                                           final List<DynamicAlertingLevel> inputLevels,
                                           final String message,
                                           final String subMessage,
                                           final List<String> tags,
                                           final List<String> alertSenders) {
        ...
        List<DynamicAlertingLevel> levels = new ArrayList<>(inputLevels == null ? List.of() : inputLevels);
        ...
        timeValues.sort((ref, value) -> Long.compare(value.getTime(), ref.getTime()));
        List<AlertThreshold> alertThreshold = checkAlertsLevel(timeValues, levels);
    ...
    //TODO: [#315] refoctor this method
    @SuppressWarnings({"java:S3776"})
    private AlertThreshold resolveThresholdAlert(final List<TimeValue> timeValues, final DynamicAlertingLevel level) {
        AlertThreshold result     = null;
        final int      delay      = level.getActivationDelay() == 0 ? 1 : level.getActivationDelay();
        String         targetName = null;
        if (timeValues.size() < delay) {
            return result;
        }
        ...
        if (found.size() == delay) {
            result = new AlertThreshold(targetName, found.getLast(), level.getLevel(), delay, level);
        }
    ...
    // TOOLS & BUILDER
    // =========================================================================
    private AlertingResult buildAlert(final AlertThreshold alertThreshold,
        ...
        return AlertingResult.builder()
                             .alerteName(event.getName())
                             .levelType(alertThreshold.getLevel())
                             .message(message)
                             .subLabel(subMessage)
                             .created(System.currentTimeMillis())
                             .duration(dynamicLevel.getDuration())
                             .providers(alertSenders)
    ...
    private List<List<TimeValue>> orderTimeValues(final List<TimeValue> timeValues) {
        ...
        for (final Map.Entry<String, List<TimeValue>> entry : buffer.entrySet()) {
            ...
            values.sort((ref, value) -> Long.compare(ref.getTime(), value.getTime()));
            result.add(values);
        }
