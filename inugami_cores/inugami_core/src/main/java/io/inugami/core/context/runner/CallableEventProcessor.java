...
public class CallableEventProcessor {
    ...
    // ATTRIBUTES
    // =========================================================================
    private static final ProviderFutureResult EMPTY_PROVIDER_FUTUR = new ProviderFutureResultBuilder().addData(new Json("null"))
                                                                                                      .build();
    ...
    // CONSTRUCTORS
    // =========================================================================
    public CallableEventProcessor(final Plugin plugin,
        ...
        simpleEventProcessor = CallableSimpleEventProcessor.builder()
                                                           .plugin(plugin)
                                                           .context(context)
                                                           .timeout(timeout)
                                                           .build();
    ...
    public ProviderFutureResult process(final Event event) {
        ...
        final Provider genericProvider = !event.getProvider()
                                               .isPresent() ? null : context.getProvider(event.getProvider().get());
        ...
        return new EventCompositeResult(null, null, resultData.getData()
                                                              .get(), event, channel, event.getScheduler(), alerts);
    }
    ...
    private Callable<ProviderFutureResult> buildTargetTask(final Provider genericProvider, final TargetConfig target) {
        ...
        Asserts.assertNotNull(String.format("no provider define for event : %s", target.getName()), provider);
    ...
    private Provider extractProviderAggregator(final Event event) {
        ...
        if (event.getProvider().isPresent()) {
            ...
            Asserts.assertNotNull(
                    "Please check provider configuration on event :" + event.getName() + "!", providerName);
        }
