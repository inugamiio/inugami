 ...
 * @see io.inugami.core.context.plugins.PluginBuilder
 * @since 4 oct. 2016
 */
@SuppressWarnings({"java:S3655", "java:S6355", "java:S1123", "java:S2139", "java:S1133"})
@Deprecated
public class ConfigurationHolder {
    ...
    public Optional<List<Plugin>> getPlugins() {
        return Optional.ofNullable(pluginsRefs);
    }
    ...
    public void callAllListener(final EngineListenerProcessor listenerProcessor) {
        ...
        if (getPlugins().isPresent()) {
            pluginsRefs.stream()
                       ...
                       .filter(plugin -> plugin.getListeners().isPresent())
                       .forEach(plugin -> plugin.getListeners().get().forEach(listenerProcessor::process));
        }
        // @formatter:on
    }
    ...
    public Optional<List<Provider>> getProviders() {
        ...
        // @formatter:off
        if (getPlugins().isPresent()) {
            pluginsRefs.stream().filter(Plugin::isEnabled).filter(
