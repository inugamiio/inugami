...
public class GitlabMergeRequests implements JsonObject {
    ...
    public <T extends JsonObject> T convertToObject(final byte[] data, final Charset charset) {
        ...
        List<GitlabMergeRequest> currentMergeRequests;
        ...
        if (data != null) {
            ...
            if (!json.isEmpty()) {
                try {
                    currentMergeRequests = new JSONDeserializer<List<GitlabMergeRequest>>().use(null,
                                                                                                ArrayList.class)
                                                                                           .use("values",
                                                                                                GitlabMergeRequest.class)
                                                                                           .deserialize(json);

                    result = new GitlabMergeRequests(currentMergeRequests);
