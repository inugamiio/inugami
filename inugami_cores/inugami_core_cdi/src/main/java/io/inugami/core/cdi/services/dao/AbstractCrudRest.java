...
public abstract class AbstractCrudRest<E extends Identifiable<PK>, PK extends Serializable> {
    ...
    private CrudSecurityHandler<E> createGrantAllProxy() {
        return new ProxyBuilder<CrudSecurityHandler<E>>().addInterface(CrudSecurityHandler.class)
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public int count(@QueryParam("fitlers") final String rawFilters) throws DaoException {
    ...
    @Path("{uid}")
    @Produces(MediaType.APPLICATION_JSON)
    public E get(@PathParam("uid") final String uid) throws TechnicalException {
    ...
    @DELETE
    @Produces(MediaType.APPLICATION_JSON)
    public Response delete(@PathParam("uid") final String uid) throws TechnicalException {
        SecurityTools.checkInjection(uid);
