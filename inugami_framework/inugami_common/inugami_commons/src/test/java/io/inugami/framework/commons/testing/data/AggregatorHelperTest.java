 ...
 */
package io.inugami.framework.commons.testing.data;

import io.inugami.framework.commons.data.AggregatorHelper;
import io.inugami.framework.interfaces.models.Record;
...
class AggregatorHelperTest {
    ...
    void testSumGenericType() {
        ...
        final Byte byteR = aggregator.sumGenericType((byte) 1, (byte) 1);
        assertEquals(Byte.valueOf((byte) 2), byteR);

        final Short shortR = aggregator.sumGenericType((short) 1, (short) 1);
        assertEquals((short) 2, (short) shortR);

        final Integer intR = aggregator.sumGenericType(Integer.valueOf(1), Integer.valueOf(1));
        assertEquals(2, (int) intR);

        final Long longR = aggregator.sumGenericType(1L, 1L);
        assertEquals(2L, (long) longR);

        final Float floatR = aggregator.sumGenericType(1.0f, 1.0f);
        assertEquals(2.0f, floatR, 0.0001f);

        final Double doubleR = aggregator.sumGenericType(1.0, 1.0);
