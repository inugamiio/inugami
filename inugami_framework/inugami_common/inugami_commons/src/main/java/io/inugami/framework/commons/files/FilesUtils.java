 ...
 * @since 12 janv. 2017
 */
@SuppressWarnings({"java:S2093", "java:S2479", "java:S5361", "java:S5869", "java:S6397", "java:S2139"})
@Slf4j
public class FilesUtils {
    ...
    public static String readFileFromClassLoader(final String resourceName, final Charset charset) {
        String result = null;
        try {
            final byte[] data = FilesUtils.readFromClassLoader(resourceName);
    ...
    public String read(final File file, final int bufferSize, final String encoding) throws IOException {
        ...
        if (log.isDebugEnabled()) {
            log.debug("done reading : nb chars : {}", data.length());
        }
    ...
    public static void readLineByLine(final File file, final Consumer<String> consumer) throws IOException {
        assertCanRead(file);
        Asserts.assertNotNull("consumer is mandatory!", consumer);
    ...
    public static Map<String, String> readProperties(final String content) {
        Asserts.assertNotNull("properties content mustn't be null!", content);
    ...
    private StringBuilder processReading(final File file,
        ...
        try {
            ...
            while (inChannel.read(buffer) > 0) {
                ...
                buffer.clear();
                if ((fileRead % MEGA) == 0) {
                    log.info("read rest : {}", fileSize - fileRead);
                }
            }
        } catch (final IOException e) {
    ...
    public static void write(final String content, final File file, final String encoding) throws FilesUtilsException {
        ...
        try {
            if (content != null) {
                ...
                log.info("write file : {}", file.getCanonicalFile().getAbsolutePath());
            }
        } catch (final IOException e) {
            log.error(e.getMessage());
        } finally {
    ...
    public static boolean isAbsoluteFile(final String path) {
        Asserts.assertNotEmpty("Path mustn't be empty!", path);
    ...
    public static void unzip(final File zipFile, final File destination) throws IOException {
        ...
        log.info("unzip {} to {}", zipFile, destination);
    }
