 ...
 */
package io.inugami.framework.api.connectors.internal;
...
import io.inugami.framework.interfaces.connectors.CredentialsProvider;
import io.inugami.framework.interfaces.connectors.HttpRequest;
import io.inugami.framework.interfaces.exceptions.Asserts;
import io.inugami.framework.interfaces.exceptions.connector.*;
...
import io.inugami.framework.interfaces.tools.strategy.DefaultStrategy;
import io.inugami.framework.interfaces.tools.strategy.Strategy;
import io.inugami.framework.interfaces.tools.strategy.StrategyException;
import lombok.*;
...
@SuppressWarnings({"java:S1845", "java:S5527", "java:S1604", "java:S1874", "java:S1130"})
@Slf4j
...
class HttpBasicConnectorDelegateUtils {
    ...
    static String buildRequest(final String baseUrl,
                               final List<Tuple<String, String>> parameters) {
    ...
    static void defineHearders(final HttpRequest request, final BiConsumer<String, String> consumer) {
        ...
        final Map<String, String> tracking = MdcServiceSpiFactory.getInstance().getTrackingInformation();
    ...
    public static ConnectorException resolveException(final int statusLine) {
        for (final Strategy<Integer, ConnectorException> strategy : EXCEPTIONS_STRATEGIES) {
    ...
    static ReadResponseResultDTO readResponse(final CloseableHttpResponse response,
        ...
        byte[] rawData = null;
        try {
            rawData = readData(response.getEntity().getContent(), url);
        } catch (final IOException e) {
    ...
    static void assertResponseOk(final HttpResponse response) throws ConnectorException {
        ...
        if (statusCode >= 400) {
            final ConnectorException exception = resolveException(response.getCode());
            if (exception != null) {
                throw exception;
            }
        }
