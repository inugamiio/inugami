...
public class UnitTestHelperLogs {
    ...
    static void assertLogs(final VoidFunctionWithException process, final Class<?> objClass, final String logs, final LineMatcher... matchers) {
    ...
    static void assertLogsIntegration(final VoidFunctionWithException process, final Class<?> objClass, final String logs, final LineMatcher... matchers) {
        assertLogs(AssertLogContext.builder()
                                   .process(process)
                                   .addClass(objClass)
    ...
    static void assertLogs(final VoidFunctionWithException process, final String pattern, final String logs, final LineMatcher... matchers) {
        assertLogs(AssertLogContext.builder()
                                   .process(process)
                                   .addPattern(pattern)
                                   .lineMatchers(Arrays.asList(matchers))
    ...
    static void assertLogsIntegration(final VoidFunctionWithException process, final String pattern, final String logs, final LineMatcher... matchers) {
        assertLogs(AssertLogContext.builder()
                                   .process(process)
                                   .addPattern(pattern)
                                   .integrationTestEnabled()
    ...
    private static void validateContext(final AssertLogContext context) {
        if ((context.getClasses() == null || context.getClasses().isEmpty()) && (context.getPatterns() == null || context.getPatterns().isEmpty())) {
    ...
    private static List<LogListener> createListeners(final AssertLogContext context, final Consumer<BasicLogEvent> appender) {
    ...
    private static void assertLogsContent(final List<BasicLogEvent> logs,
        ...
        final LineMatcher[] matchers = Optional.ofNullable(context.getLineMatchers()).orElse(new ArrayList<>()).toArray(new LineMatcher[]{});
